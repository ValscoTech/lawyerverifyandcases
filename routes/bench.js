...
router.post('/fetchBenches', async (req, res) => {
  try {
    ...
    req.session.save((err) => {
      ...
      // --- MODIFIED: Include session cookies and initial eCourts cookies in the response ---
      res.json({
        benches: benches,
        sessionID: getSessionCookie(req), // Include the Express session ID
        initialEcourtsCookies: req.session.initialEcourtsCookies // Include the captured eCourts cookies
      });
      // --- END MODIFIED ---
    });
